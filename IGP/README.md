...
The IGP model is an ecological system in which species compete and prey upon one another simultaneously. It exhibits bistability and fold bifurcations in parameter space, providing a higher-dimensional ecological analogue of the normal forms studied elsewhere in this repository.
...
- A basal resource  
- An intermediate consumer  
- A top predator  

The governing system consists of three coupled nonlinear ODEs of the form:

\[
\begin{aligned}
\dot{R} &= \text{resource growth} - \text{consumption} \\
\dot{C} &= \text{consumer gain} - \text{mortality} - \text{predation} \\
\dot{P} &= \text{predator gain} - \text{mortality}
\end{aligned}
\]
...
- **Horizontal axis:** (e.g.) enrichment or carrying capacity parameter  
- **Vertical axis:** (e.g.) predator mortality or attack rate  

(Replace with exact parameter names used in your implementation.)

The ranges used in the paper should be inserted here.
...
Because the IGP model is three-dimensional, equilibria are points in \((R, C, P)\)-space.
...
```matlab
igp_make_vb("high","param1",[min1 max1],"param2",[min2 max2],[350 350]);
```

(Replace `param1` and `param2` with the parameter names used in the code.)
...
- Resilience is not determined solely by proximity to the bifurcation boundary.
